...
def train(args, txt_src, txt_tgt):
    ...
    if args.issave:
        file0 = osp.join(args.mm_dir, args.log + ".npz")
        np.savez(file0, y=y.cpu().numpy(), py=py.cpu().numpy(), score=score.cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office-home', choices=['pacs', 'office-caltech', 'office-home'], help="The dataset or source dataset used")
    ...
    for i in range(len(names)):
        ...
        if args.dset == 'pacs':
            args.t_dset_path = folder + args.dset + '/' + names[args.t] + '_test_kfold.txt'
