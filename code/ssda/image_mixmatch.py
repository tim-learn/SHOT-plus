...
def split_target(args):
    ...
    test_set = ImageList(open(args.tu_dset_path).readlines(), transform=test_transform)
    dset_loaders = {}
    ...
    ori_target = open(args.tu_dset_path).readlines()
    new_tar = []
    new_src = []
...
def data_load(args, txt_src, txt_tgt):
    ...
    dsets = {}
    txt_src.extend(open(args.tl_dset_path).readlines())
    ...
    dsets["test"] = ImageList(open(args.tu_dset_path).readlines(), transform=test_transform)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office-home', choices=['DomainNet126', 'office', 'office-home'], help="The dataset or source dataset used")
    ...
    folder = '/Checkpoint/liangjian/tran/data/ssda/'
    ...
    args.output_dir = osp.join(args.output_tar, 'shot_' + str(args.shot), args.dset, args.name)
    ...
    args.mm_dir = osp.join(args.output, 'shot_' + str(args.shot), args.dset, args.name)
